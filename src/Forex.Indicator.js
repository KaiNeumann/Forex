...
K.merge(Forex.Indicator,{
	...
	, requirementsToDefinition: function(a){//converts Indicator Requirements into properIndicator Definitions (to be used with add Indicator)
		return a.map(function(el){
			...
			if(typeof el.definition==="string"){ //a string is shortcut to load a predefined indicator definition with that name
				defintition = Forex.Indicator.getDefinition(
					el.definition.indexOf("@")==0 ? o.parameter[ el.definition.substring(1) ] : el.definition //could also be read from a parameter
				);
			} else {
				definition = el.definition;		//of course we can define a whole indicator there as well
			}
