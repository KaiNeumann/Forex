	...
	_init:function(o){
		K.merge(this,{
			...
			, criteria:		"balance"		//which criteria will be used to sort iteration results and shown
				//TODO needs to be changable in UI as well
		});
		this.build();
		return this;
	}
		...
		var paramDiv = $KDE("div", {"id":this.id,style:"float:right;"}
			...
			*/			
			...
			, $KDE("input", {type:"button", value:"Iterate!", id:this.id+"_button", 
				click:function(){
					var r = _this.o.iterate();
					_this.o.printStatisticBestIteration("result",r);
					_this.o.plotBestIteration("plot",r);
					_this.o.outputIterationGraphs("plot2",r);
				}}
			)
			, " or "
			, $KDE("input", {type:"button", value:"Run once with eventlog", id:this.id+"_button", 
					click:function(){
						var r = _this.o.runonce("result");
					}} 
			)
		);
