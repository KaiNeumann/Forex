...
Forex.Strategy.Statistic = K.Base.subclass({
	...
	, getSortedResults:	function(criteria){
		return this.results.sort(function(a,b){ return a[criteria] > b[criteria] ? 1 : (a[criteria] < b[criteria] ? -1 :0); });
	}
	, getSummary: function(){
    ...
    , plot:function(criteria){
        ...
        $.plot(strategyplot1, [ 
            { data: sorted.map(function(t,i){ return [i,t[criteria]]; })
                        ,label:"Iterations "+criteria,bars:{show:true,lineWidth:.5}}
            ,{ data: sorted.map(function(t,i){ return [i,Math.min(5,Math.abs(t.statistic.t_test))*t.statistic.t_test.sign()]; })
                        ,label:"Iterations T-Test",bars:{show:true,lineWidth:.5}, yaxis: 2}
